...
const MockApi = () => {
  return (
    <>
      <p>
        A mock API is an API that responds data just like a real API to do, however it is just imitating the data & response. This is extremely useful in many cases, like when the
        API you rely on isn&apos;t fully developed yet, or simulate API calls on unit testing.
      </p>
      <p>
        Byol uses{' '}
        ...
        for mocking API calls, all the api interaction in our demo are work under miragejs.
      </p>
      <div className='mt-10' id='disableMockApi'>
        <h5>Disabled mock api</h5>
        <p className='mt-1'>
          Mock api was enabled by default in our <strong>starter-kit</strong>, you can turn in on via setting <code>enableMock</code> field to false in{' '}
          <code>src/configs/app.config.ts</code> to turn off the mock api.
        </p>
    ...
    enableMock: false
      ...
      <div className='mt-10' id='usingMockApi'>
        ...
        <p className='mt-1'>If you have intent to use mock api, you can follow the steps below to create a fake db data & api</p>
        <ol>
          <li>
            <strong>Setup</strong>
            <p className='mt-1'>
              Visit <code>src/mock/index.ts</code>, and here is the overall configuration of mirage
            </p>
